...
| **elementor-mcp** | construir páginas | **HTTP remoto** → `https://factum-omibu.es/newomibu/wp-json/mcp/elementor-mcp-server` (plugin MCP dentro del propio WordPress, auth Basic) | ✅ cualquier entorno con el header |
...
2. Conectar **elementor-mcp** MCP (HTTP al endpoint `…/wp-json/mcp/elementor-mcp-server`
   con header `Authorization: Basic <application-password>`).
